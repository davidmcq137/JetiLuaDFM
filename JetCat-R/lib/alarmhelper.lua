...
function alarmhelper.Haptic(tmpConfig)
    ...
    if(tmpConfig.enable and enableAlarmHaptic_R) then
...
function alarmhelper.Audio(tmpConfig)
    ...
    if(tmpConfig.enable and enableAlarmAudio_R) then
        -- If audio file associated with status, we play it
        if(tmpConfig.file) then
            system.playFile(string.format("/Apps/JetCat-R/audio/%s", tmpConfig.file), AUDIO_QUEUE);

            if(io.open(string.format("/Apps/JetCat-R/audio/%s", tmpConfig.file), "r")) then
