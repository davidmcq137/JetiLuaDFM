...
local function DrawFuelGauge(percentage, ox, oy) 
    ...
    --lcd.drawText(4+ox,15+oy, config_R.fuel.tanksize, FONT_BOLD)
    --lcd.drawText(1+ox,49+oy, string.format("Fulltank: %.1f",tonumber(config_R.fuel.tanksize/1000)), FONT_BOLD)
end
...
function telemetry_window1.show(width, height) 
  ...
  if(SensorID ~= 0) then
    if(sensorsOnline_R > 0) then
		...
		-- fuel
        if(config_R.converter.sensormap.fuel) then
          if(SensorT_R.fuel.percent) then
            if(SensorT_R.fuel.percent > 20) then
              DrawFuelGauge(SensorT_R.fuel.percent, 1, 0)
            else
              DrawFuelLow(SensorT_R.fuel.percent, 1, 0)
            end
          end
        end

        -- turbine state
        if(config_R.converter.sensormap.status) then
          if(SensorT_R.status.text) then
            DrawTurbineStatus(SensorT_R.status.text, xs, 0)
          else
        ...
        -- Restvol
        if(config_R.converter.sensormap.fuel and SensorT_R.fuel.sensor.value) then		  
		    lcd.drawText(xs,1+37, "Fuel", FONT_MINI)  
			lcd.drawText(xs,12+37,  string.format("%1.0f%s",SensorT_R.fuel.sensor.value,"ml"), FONT_BOLD)
        end
		
        -- pumpv
        if(config_R.converter.sensormap.pumpv and SensorT_R.pumpv.sensor.value) then		  
		    lcd.drawText(xs+65,1+37, "Pump", FONT_MINI)  
			lcd.drawText(xs+65,12+37,  string.format("%.1f%s",SensorT_R.pumpv.sensor.value,"V"), FONT_BOLD)
        end		
		
		--battery V
        if(config_R.converter.sensormap.ecuv and SensorT_R.ecuv.sensor.value) then		  			
			lcd.drawText(xs,17, string.format("%.1f%s",SensorT_R.ecuv.sensor.value,"V"), FONT_BOLD)
        end
		
		
		if(config_R.converter.sensormap.battcapa and SensorT_R.battcapa.sensor) then
			-- mit capa, 1s abwechselnd
			
			if( system.getTime() % 2 == 0) then -- switch every second
				-- curr zeigen
				if(config_R.converter.sensormap.engcurr and SensorT_R.engcurr.sensor) then		
					lcd.drawText(xs+42,17, string.format("%.1f%s",SensorT_R.engcurr.sensor.value,"A"), FONT_BOLD)  --35
				end
			else
				-- capa anz
				lcd.drawText(xs+42,17, string.format("%1.0f%s",SensorT_R.battcapa.sensor.value,"mAh"), FONT_BOLD)  --35			end		
			end
		else
    ...
    DrawTurbineStatus("NO config_R", xs, 0)
  end
end
