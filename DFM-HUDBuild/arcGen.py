...
# usage: python3 arcGen.py wid, hgt, x0, y0, ro, ri, minA, maxA, file
#
...
#       ro is the radius of the outer arc
#       ri is the radius of the inner arc
...
xo = ro * math.sin(math.radians(minA))
yo = ro * math.cos(math.radians(minA))

xi = ri * math.sin(math.radians(minA))
yi = ri * math.cos(math.radians(minA))	

draw.line((x0 + xi, h - (y0 + yi), x0 + xo, h - (y0 + yo)), fill=(col,col,col), width=linewidth)
#draw_line_antialiased(draw, im, x0 + xi, h - (y0 + yi), x0 + xo, h - (y0 + yo), (255,255,255,255))

xo = ro * math.sin(math.radians(maxA))
yo = ro * math.cos(math.radians(maxA))

xi = ri * math.sin(math.radians(maxA))
yi = ri * math.cos(math.radians(maxA))	
...
deltax = (width - 2*ri) / 2.0
deltay = (height- 2*ri) / 2.0
shape = [(deltax,deltay),(width-deltax, height-deltay)]
draw.arc(shape, start=minA-90, end=maxA-90, fill=(col,col,col))

deltax = (width - 2*ro) / 2.0
deltay = (height- 2*ro) / 2.0
shape = [(deltax,deltay),(width-deltax, height-deltay)]
draw.arc(shape, start=minA-90, end=maxA-90, fill=(col,col,col))
