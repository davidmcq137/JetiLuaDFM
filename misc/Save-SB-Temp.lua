...
local function intbxChanged(value)
   print("intbx value:", value)
   print("Sform:", SForm)
end
...
local function initForm(subForm)
   ...
   if subForm == 1 then
      ...
      for j=1,8,1 do
	 form.addRow(2)
	 form.addLink((function() form.reinit(j+1) end), {label="Probe "..j.." >>"})
      end
      form.addRow(1)
      ...
      local intbx=1
      form.addRow(2)
      form.addLabel({label="Add test integer", width=220})
      form.addIntbox(intbx,1,8,1,0,1,intbxChanged)
      
      form.addRow(1)
      local k=subForm-1
      form.addLabel({label="SB-Temp SF"..k,font=FONT_MINI, alignRight=true})
   end
end
