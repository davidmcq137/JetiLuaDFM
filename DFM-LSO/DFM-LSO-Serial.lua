...
local function binom(n, k)
   ...
   --print("binom: n, k=", n, k)
   if (n == MAXTABLE-1) and binomC[k] then
      --print("B:", binomC[k])   
      return binomC[k] 
   end
...
--      speed = 40 + 80 * (math.sin(.3*debugTime) + 1)
   ...
   if fd and rlhDone then
      if (system.getTimeCounter() - logItems.sysStartTime) >= (tonumber(timS) - timSn)/1. and blocked then
	 blocked = false
	 ...
	 local xxx = logItems.vals['CTU_EGT'] or 0
	 local yyy = logItems.vals['CTU_Fuel_remaining'] or 0
	 local zzz = logItems.vals['CTU_RPM'] or 0
	 
	 --print("xxx,yyy:", xxx, yyy)

	 --print("log:", timS, latitude, longitude, altitude, speed, courseGPS)
	 
	 if lastlat ~= latitude and lastlon ~= longitude then
	    local latlonstr = string.format("(Pos:%4.8f$%4.8f)", latitude, longitude)
		local latstr = string.format("(Lat:%4.8f)", latitude)
		local lonstr = string.format("(Lon:%4.8f)", longitude)
		local altstr = string.format("(Alt:%4.2f)", altitude)
		local spdstr = string.format("(Spd:%4.2f)", speed)
		local xxxstr = string.format("(EGT:%4.2f)", xxx)
		local yyystr = string.format("(Frm:%4.2f)", yyy)
		local zzzstr = string.format("(RPM:%4.2f)", zzz)
		...
		local written1 = serial.write(sidSerial, latlonstr..altstr..spdstr)
		local written2 = serial.write(sidSerial, xxxstr..yyystr..zzzstr)
	 
		
		if written1 and written2 then  
			local written = written1 + written2
			local te = (system.getTimeCounter() - sidTime0) / 1000
			--print("i, Number of bytes written: ", i, written) 
			serialbb = serialbb + written
			serialii = serialii + 1
			if serialii%100 == 0 then print("i, bytes, times, bytes/sec: ", serialii, serialbb, te,serialbb/te) end
		else     
			print("Error writing to serial port at i=", i)   
		end
	 end
...
--   end
   ...
   readSensors()
	...
	sidSerial = serial.init("COM6",9600) 
