...
local function drawHeading()
  ...
  lcd.setClipping(math.floor(c1),math.floor(c2),math.floor(c3),math.floor(c4))
  if ii > 50 then
     print(math.floor(c1),math.floor(c2),math.floor(c3),math.floor(c4))
     ii = 0
  end
  
  
  --lcd.drawLine(colHeading - 70, rowHeading, colHeading + 70, rowHeading)
  ...
  for index, point in pairs(parmHeading) do
    ...
    if wrkHeading < -180 then wrkHeading = wrkHeading + 360 end
    delatX = floor(wrkHeading / 1.6 + 0.5) - 1 -- was 2.2

    if delatX >= -64 and delatX <= 62 then -- was 31
      if point[3] then
	 lcd.drawText(colHeading + delatX - 4, rowHeading - 16, point[3], FONT_NORMAL)
	 --print("dT: ", colHeading + delatX-4, rowHeading-16, point[3])
      end
      if point[2] > 0 then
...
local function drawAltitude()
  ...
  lcd.drawText(colAlt+2, heightAH, "ft", FONT_MINI)
...
local function drawSpeed() 
  ...
  delta = speed % 10
  deltaY = 1 + floor(2.4 * delta)
  lcd.drawText(colSpeed-30, heightAH, "mph", FONT_MINI)
...
local test = true                -- test flag (true to simulate telemetry data)
