...
local function initForm(sf)
   ...
   elseif sf == 104 then
      ...
      form.addRow(2)
      form.addLabel({label="Enable dial and chart updates", width=240})
      swtCI.holdDialData = form.addInputbox(switches.holdDialData, false,
					    (function(x) return changedSwitch(x, "holdDialData") end))

      form.addRow(2)
      form.addLabel({label="Enable dial updates only", width=240})
      swtCI.holdChartData = form.addInputbox(switches.holdChartData, false,
					    (function(x) return changedSwitch(x, "holdChartData") end))
      form.addRow(2)
...
local function loop()
   ...
   if now > lastauto + 200 then
	 ...
	 for i = 1,4,1 do
	    ...
	    if tele[win].sensorAuto[i] == "On" and twVmin and twVmax then
	       if twVmin < twmin then
		  tt, twmin, twmax = heckbert(twVmin, twmax)
		  ...
		  print("heckbert", win, i, tt, twmin, twmax)
	       ...
	       if twVmax > twmax then
		  tt, twmin, twmax = heckbert(twmin, twVmax)
		  tele[win].sensorMin[i] = twmin / tele[win].sensorMinMult[i]
		  tele[win].sensorMax[i] = twmax / tele[win].sensorMaxMult[i]
		  print("heckbert", win, i, tt, twmin, twmax)
	       end
   ...
   local swhc = system.getInputsVal(switches.holdChartData)
   ...
   for i=1,2,1 do
	 ...
	 if idx > 1 then
	    ...
	    if sensor and sensor.valid then
		  ...
		  if not swhc or (swhc and swhc == 1) then
		     sample = true
...
local function init()
   ...
   -- and repair any userdata nulls in sensorMin/MaxWarn (json encode/decode issue)
