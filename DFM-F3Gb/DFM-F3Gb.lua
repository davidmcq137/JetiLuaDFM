--[[
   ...
   This is the Basic (DFM-F3Gb) version with minimal function per requirements 
   from Jeroen Smits
   ----------------------------------------------------------------------------
   
--]]

local F3GVersion = "1.1"
...
local function keyForm(key)
   ...
   if zeroLatString and zeroLngString and rotA and gpsScale and (key >= KEY_1) and (key <= KEY_4) then
      ...
      local fp = io.open(prefix() .. 'Apps/DFM-F3Gb/GPS.jsn', "w")
      if fp then
   ...
   if sf == 1 then
	 ...
	 form.addInputbox(ctl[ctl[i].var], true, (function(x) return ctlChanged(x, ctl, ctl[i].var) end) )
      ...
      form.addLink((function()
	       ...
	       --perpA = nil
	       --perpB = nil
	       io.remove(prefix() .. 'Apps/DFM-F3Gb/GPS.jsn')
...
local function init()
   ...
   local jtext = io.readall(prefix() .. 'Apps/DFM-F3Gb/GPS.jsn')
   ...
   system.registerForm(1, MENU_APPS, "F3Gb", initForm, keyForm, formTele)
   system.registerTelemetry(1, "F3Gb Status", 2, printTele)
   
   readSensors(telem)

   print("DFM-F3Gb: gcc " .. collectgarbage("count"))
