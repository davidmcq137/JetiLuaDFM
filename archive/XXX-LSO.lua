...
--		   toYPixel(rwy[j].y, map.Ymin, map.Yrange, windowHeight))
  ...
  --[[
   if #nfc > 0 then
      setColorNoFly()
      for j=1, #nfc do
	 lcd.drawCircle(toXPixel(nfc[j].x, map.Xmin, map.Xrange, windowWidth),
			toYPixel(nfc[j].y, map.Ymin, map.Yrange, windowHeight),
			nfc[j].r * windowWidth /  map.Xrange)
      end
   end
...
--   lcd.drawText(70-lcd.getTextWidth(FONT_MINI, text) / 2, 42, text, FONT_MINI)
   ...
   for i=1, #xtable do -- if no xy data #table is 0 so loop won't execute 
	 ...
	 if (dl >= 0 and dr <= 0) and (not takeoff.NeverAirborne) then
      ...
      if i == #xtable then
	 print("geo, geo.fields, iField", geo, geo.fields, iField)
	 if not geo.fields[iField].NoFly or #geo.fields[iField].NoFly < 3 then
	    noFlyP = false
	 else
	    noFlyP = isInside (poi, #poi, {x=xtable[i], y=ytable[i]})
	 end
	 
	 if not geo.fields[iField].NoFlyCircle then
	    noFlyC = false
	 else
	 ...
	 if geo.fields[iField].noFlyZone and geo.fields[iField].noFlyZone == "Outside" then
	    noFly = not noFly
	 end
